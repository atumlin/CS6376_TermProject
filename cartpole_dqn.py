...
class DQNAgent:
    ...
    def state_shape(self, states):
        ...
        return states.reshape(-1, 4)  # Reshape to have 4 features
    ...
    def train(self):
        ...
        if len(self.D) >= self.batch_size: 
            ...
            qvalue = self.model.Qpolicy.predict(s)

            s_ = self.state_shape([row[3] for row in SARS])
            future_qvalue = self.model.Qtarget.predict(s_)
    ...
    def training(self):
        ...
        for episode in tqdm(range(1, self.episodes+1), ascii = True, unit = 'episode'): 
            ...
            while done != True:
                ...
                # Execute the action and get the step output
                step_output = self.env.step(a)

                # Unpack the standard four values
                s_, r, done, _ = step_output[:4]
